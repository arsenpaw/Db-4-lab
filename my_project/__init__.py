...
def init_autodoc_swagger(app: Flask) -> None:
    """
    Ініціалізує Flasgger (Swagger UI + OpenAPI 3).
    - Автоматично генерує /apidocs (UI) і /apispec_1.json (специфікацію).
    - Підтримує опис ендпоінтів через YAML у докстрінгах.
    """
    # Базові налаштування Swagger UI
    app.config.setdefault("SWAGGER", {
        "uiversion": 3,            # Swagger UI v3
        "title": "Pets Shop API",  # заголовок у UI
        "openapi": "3.0.3",
    })

    # Опційний шаблон (OpenAPI 3) — додайте тут безпеку, сервери тощо
    swagger_template = {
        "openapi": "3.0.3",
        "info": {
            "title": "Pets Shop API",
...
def _init_swagger(app: Flask) -> None:
    api = Api(
        ...
        description="Autogenerated Swagger docs",
        doc="/swagger"   # Swagger UI auto-generated
    )
